...
int16_t IMU::readX() { return readAxis(OUT_X); }

int16_t IMU::readY() { return readAxis(OUT_Y); }

int16_t IMU::readZ() { return readAxis(OUT_Z); }
...
void IMU::readXYZ(int16_t* x, int16_t* y, int16_t* z) {
    ...
    WIRE_IMU.write(
        OUT_X
        | (1 << 7)); // assert MSB to enable register address auto-increment
    WIRE_IMU.endTransmission();
